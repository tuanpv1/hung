 ...
 */
class AboutController extends BaseBEController
{
    /**
     * @inheritdoc
     */
    ...
    {
        $model = News::findOne(['type' => News::TYPE_ABOUT,'status' => News::STATUS_ACTIVE]);
        if($model){
    ...
    {
        // Kiểm tra có bài viết giới thiêu chưa
        $model = News::findOne(['type' => News::TYPE_ABOUT,'status' => News::STATUS_ACTIVE]);
        if($model){
            return $this->redirect(['view']);
        }
        // Nếu không thì mới cho tạo mới
        $model = new News();
